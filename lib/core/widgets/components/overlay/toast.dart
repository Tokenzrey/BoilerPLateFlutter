...
class _ToastLayerState extends State<ToastLayer> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var locationEntry in entries.entries) {
      ...
      int toastIndex = 0;
      var collapsedOffset = widget.collapsedOffset ?? (const Offset(0, 12));
      var padding = widget.padding?.optionallyResolve(context) ??
          (const EdgeInsets.all(24));
      var toastConstraints =
          widget.toastConstraints ?? BoxConstraints.tightFor(width: 320);
...
class _ToastEntryLayoutState extends State<ToastEntryLayout> {
  ...
  Widget buildToast(
    ...
    Offset expandingShift = Offset(
      previousAlignment.x * (16) * expandProgress,
      previousAlignment.y * (16) * expandProgress,
    );
